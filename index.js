...
class NodeClam {
    ...
    async init(options={}, cb) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (Object.prototype.hasOwnProperty.call(options, 'clamdscan') && Object.keys(options.clamdscan).length > 0) {
    ...
    scan_files(files=[], end_cb=null, file_cb=null) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const local_scan = async () => {
                ...
                // Build the actual command to run
                const command = `${self.settings[self.scanner].path} ${self._build_clam_args(items).join(' ')}`;
                if (self.settings.debug_mode)
                    if (self.settings.debug_mode) console.log(`${self.debug_label}: Configured clam command: ${command}}`);
            ...
            const do_scan = async files => {
                ...
                if (file_cb && typeof file_cb === 'function') {
                    ...
                    while (files.length > 0) {
                        ...
                        chunk_results_mapped.forEach(v => file_cb(null, v[0], v[1]));
    ...
    scan_dir(path='', end_cb=null, file_cb=null) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (this.settings.scan_recursively === true && (typeof file_cb === 'function' || !has_cb)) {
                try {
                    ...
                    if (stderr) {
                        ...
                        return (has_cb ? end_cb(null, [], []) : resolve({stderr, path, is_infected: null, good_files: [], bad_files: [], viruses: []}));
                    }
            ...
            // Clamdscan always does recursive, so, here's a way to avoid that if you want (will call `scan_files` method)
                ...
                try {
                    const all_files = (await fs_readdir(path)).filter(async file => (await fs_stat(file)).isFile());
